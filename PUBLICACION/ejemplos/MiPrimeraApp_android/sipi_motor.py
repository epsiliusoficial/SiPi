...
COMANDOS_CONOCIDOS = [
    ...
    "matriz_obtener", "matriz_filas", "matriz_columnas", "crear_archivo",
    ...
    "hash_texto", "elegir_al_azar", "ventana", "boton", "etiqueta", "entrada",
...
AYUDA_COMANDOS = {
    ...
    "hash_texto": ("Calcula el hash SHA-256 de un texto (una huella digital, no reversible).", 'hash_texto contraseña -> huella'),
...
AYUDA_CASOS_DE_USO = {
    ...
    "hash_texto": "guardar una contraseña de forma segura (nunca guardar la contraseña real) o verificar que un archivo descargado no se corrompio.",
...
class SiPiError(Exception):
    def __init__(self, mensaje, pila=None):
...
PALABRAS_NIVEL_3_MEDIO = PALABRAS_NIVEL_2_FACIL | {
    ...
    "leer_archivo", "escribir_archivo", "existe_archivo", "listar_archivos",
    "json_a_texto", "texto_a_json", "hash_texto",
...
PALABRAS_NIVEL_4_DIFICIL = PALABRAS_NIVEL_3_MEDIO | {
    ...
    "hilo", "paralelo", "capturar_pantalla", "editor_visual", "compilar_a_js",
}
...
class Interprete:
    ...
    @staticmethod
    def _preprocesar_contenido(contenido_completo):
        ...
        return Interprete._inferir_fin_por_indentacion(con_indentacion)
    ...
    def cargar(self):
        ...
        self.lineas = self._preprocesar_contenido(contenido_completo)
    ...
    def _ejecutar_bloque(self, inicio, fin_idx):
        ...
        while i < fin_idx:
            ...
            try:
                ...
                raise SiPiError(f"Error en linea {num}: {e}", pila=list(getattr(self, "pila_llamadas", [])))
    ...
    def _ejecutar_linea(self, i, fin_idx, num, linea):
        ...
        if cmd == "importar":
            ...
            if m:
                ...
                lineas_modulo = self._preprocesar_contenido(contenido_modulo)
        ...
        if cmd == "crear_archivo":
            m = _m(r'^("(?:[^"]*)"|\S+)\s+("(?:.*)"|\S+)$', resto, re.DOTALL)
            if m:
                ruta = self._texto_o_variable(m.group(1))
                contenido = self._texto_o_variable(m.group(2))
    ...
    def _invocar_funcion(self, nombre, args_txt):
        ...
        nuevo_scope = self._armar_scope_de_llamada(nombre, params_info, valores_params)
    ...
    def _crear_juego_pygame(self, titulo, ancho, alto, cuerpo_lineas):
        ...
        while corriendo:
            ...
            for nombre, s in sprites.items():
                color = colores.get(s["color"], (200, 200, 200))
...
document.getElementById('guardar').addEventListener('click', async () => {{
  ...
  try {{
    ...
    def _escribir_log(self, nivel, mensaje):
...
'''
    ...
    def _lanzar_editor_visual(self):
        ...
        print("[SiPi] No se encontro ningun bloque 'ventana' ni 'pagina_web' en este programa. "
              "El editor Visual por ahora funciona sobre esos dos (mas tipos de UI se iran agregando).")
...
def main():
    ...
    if archivo == "--sin-cache":
        ...
        sin_cache = True
        if len(sys.argv) < 3:
            print("Uso: python sipi.py --sin-cache archivo.sipi")
            sys.exit(1)
        archivo = sys.argv[2]
